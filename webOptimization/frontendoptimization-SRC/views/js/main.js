...
var resizePizzas = function(size) {
  ...
  // replace document.querySelector() by document.getElementById for faster call
  ...
  // !!!removed and replace with function below!!!
  ...
  function changePizzaSizes(size) {
    ...
    // replaced randomPizzas.length with len to reduce continued check of value in loop
...
// pizzasDiv brought outside loop to reduce call to once
...
//Using reuwestAnimationFrame to improve animation efficiency
...
function updatePositions() {
  ...
  //Move phase array calulation outside main loop since it only has 5 values
  for (var p = 0; p < 5; p++) {
    phase[p] = Math.sin((Top) + (p % 5));
  }
  // replace document.querySelector() by document.getElementById for faster call
  ...
  for (var i = 0, len = items.length; i < len; i++) {
    ...
    movePizzas = items[i].basicLeft + 100 * phase[i % 5] - 1250 + 'px';
    items[i].style.transform = 'translateX(' + movePizzas + ')';
  }
...
// Dynamically calculating number of background pizzas bgPizzas based on the browser
//   resolution using window.screen.height
document.addEventListener('DOMContentLoaded', function() {
  var row = window.screen.height;
  var cols = 8;
  var s = 256;
  var bgPizzas = parseInt(cols * (row / s));
  // var elem brought outside to create the var only once
  var elem; 
  // replace document.querySelector() by document.getElementById for faster call
  // 	moved outside to 
