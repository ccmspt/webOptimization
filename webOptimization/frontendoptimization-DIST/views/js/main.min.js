...
window.animating=false;function animationReadyCheck(){if(!window.animating){window.animating=true;requestAnimationFrame(updatePositions);}}
function updatePositions(){if(!window.items){window.items=document.getElementsByClassName('mover');console.log(window.items);}
frame++;window.performance.mark("mark_start_frame");var Top=document.body.scrollTop / 1250;var phase=[];var items=[];var movePizzas;for(var p=0;p<5;p++){phase[p]=Math.sin((Top)+(p%5));}
items=document.getElementsByClassName('mover');for(var i=0,len=items.length;i<len;i++){movePizzas=items[i].basicLeft+100*phase[i%5]-1250+'px';items[i].style.transform='translateX('+movePizzas+')';}
...
window.addEventListener('scroll',animationReadyCheck);document.addEventListener('DOMContentLoaded',function(){var row=window.screen.height;var cols=8;var s=256;var bgPizzas=parseInt(cols*(row / s));var elem;var movingPizzas=document.getElementById('movingPizzas1');for(var i=0;i<bgPizzas;i++){elem=document.createElement('img');elem.className='mover';elem.src="images/pizza-compressor.png";elem.style.height="100px";elem.style.width="73.333px";elem.basicLeft=(i%cols)*s;elem.style.top=(Math.floor(i / cols)*s)+'px';movingPizzas.appendChild(elem);}
updatePositions();});